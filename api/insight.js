...
每個小標題都要出現、有實質敘述，但每段都要精簡。⚠ ${len}，為求達標請適度精簡各段、不要展開過多——這比「寫得很長」更重要。`,
  ...
  astro: (g, len, closing, astroLen) => `西洋占星——一段講一件事。有幾個配置共同指向它，就把那幾個合起來寫；若那件事是由單一配置撐起來的，一段只列一個配置也可以，不必為了湊成一組去找別的配置陪它。每一段固定三部分，各自換行：
...
最後一段的標題就寫「${closing}」。這一段不列配置那一行，也**不要寫成段落**——改用重點清單：一項一行，每一項是一句短話、或幾個詞就好，像一份最後可以快速看過的筆記大綱。前面幾段已經是長篇敘述，這裡刻意換成條列，讓讀者收尾時能一眼掃完。不要加編號或符號，直接一行一項。內容仍然是整體意義：把前面各段的核心收攏，並帶出彙整後的建議。
