...
每個小標題都要出現、有實質敘述，但每段都要精簡。⚠ ${len}，為求達標請適度精簡各段、不要展開過多——這比「寫得很長」更重要。`,
  ...
  astro: (g, len, closing) => `西洋占星——把指向同一件事的配置合成一段來寫。每一段固定三部分，各自換行：

第一行：這組配置共同指向的核心，用白話寫成一句話（讀者看得懂的生活語言，不要用星盤術語當標題）。
第二行：這組配置分別是什麼，用全形括號包起來、以｜分隔。
第三行起：分析內容。
...
最後一段的標題就寫「${closing}」，這一段不用列出配置清單，言簡意賅把前面各段核心串成一條完整的生命脈絡敘事，並列出彙整的建議。
...
function buildPrompt(action, p, seg) {
  ...
  const structs = tools.map((x) => `〔${label(x)}〕\n${TOOL_STRUCT[x](seg.groups || {}, len, closing)}`).join('\n\n');
