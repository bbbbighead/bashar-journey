...
// 只送「這九張牌是什麼、在哪個位置」，不送任何預先算好的結論。
//
// 曾經還會多送兩行，都已移除：
//   ・「中心牌（全局核心）：山」——屬於已刪除的十字法，而且與新指令衝突：
//     新指令說核心是「事件現況」那一組（4、5、6），這行說核心是位置 5 那一張，
//     同一份輸入裡出現兩個互斥的「核心」定義。
//   ・「收斂主題：阻礙與消耗（×3）…」——把九張牌的 cluster 標籤數一數就當成
//     主題，在模型讀牌之前先給它答案。而且那個統計把位置資訊丟掉了：實測那副
//     牌的三張「阻礙」落在過去與現在、三張「洞察」落在現在與未來，走向很清楚，
//     但數出來是 3:3 打平，最有用的部分反而被抹掉。
// 位置與牌義模型全都拿到了，哪一類重複、落在哪一欄由它自己讀——這才是
// 「意義由組合產生」。cluster 欄位保留，供後台分析用，不進 prompt。
function fmtLenormand(L) {
  ...
  return ['九宮格（依位置，由左至右、由上至下）：', ...lines].join('\n');
}
