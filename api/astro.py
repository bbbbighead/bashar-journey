...
# 歷史日光節約時間由 zoneinfo 的 IANA 資料庫處理。
#
# 出生資料：這支端點本身不儲存任何東西（無狀態，算完就回）。但整個流程有存——
# 前端埋點會把出生資料連同解讀結果寫進 pi:journey（見 api/track.js 的
# sanitizeBirth：日期、時間、城市、解析後的地名、時區都存，供後台重算星盤與
# 預覽當時的結果頁），首頁 fineprint 也已揭露「過程會匿名記錄」。
# 原本這裡寫「出生資料僅用於計算，不儲存」，讀起來像是全站都不存，會讓日後判斷
# 隱私邊界時誤判——所以講清楚：不存的是這支端點，不是整個系統。
# （注意這與「來訪地區」不同：那邊只存兩碼國碼、刻意不存城市，因為瀏覽所在城市
#   加上 pi_visitor_id 足以重新識別到個人。出生地是計算星盤的必要輸入，兩者的
#   取捨不一樣。）
