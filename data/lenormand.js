...
// 供整合引擎（AI 或離線模板）參考。
//
// 牌名會出現在使用者看到的解讀裡：api/insight.js 的 TOOL_STRUCT.lenormand 明確
// 要求「直接寫牌名，用 + 連接（例：船+雲：……）」。這一行原本寫「牌名絕不顯示給
// 使用者」，那是早期版本的規則，產品後來改了而註解沒跟上。
// 仍然不顯示的是 meaning／cluster 這些內部欄位——它們是給模型的素材，不是要照
// 抄給讀者的牌義條目。
//
// 欄位說明：
//   id       傳統編號 1–36
