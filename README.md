...
占卜一：雷諾曼——36 張牌全數列出（牌背朝上、順序洗亂），
      ...
      ↓
占卜二：梅花易數——報三個 1–100 的數字起卦（跳過則以時間起卦）
        → 觀測「現在處於哪個階段」
      ↓
...
- **全部實算**：行星（日月水金火木土天海冥）、真北交/南交、凱龍、平均黑月莉莉絲、穀神/智神/婚神/灶神、四軸（ASC/DSC/MC/IC）、福點、Vertex——由 Swiss Ephemeris（pyswisseph）計算，星曆檔（`ephe/`）隨 repo 部署；AI 只詮釋、不補造數據。
...
- 地點以 Open-Meteo 免費 geocoding 解析（含 IANA 時區），歷史日光節約時間由 zoneinfo 處理。**出生資料僅用於當下計算，不儲存**；後台僅記錄「是否使用占星＋太陽星座」。
...
api/astro.py             西洋占星本命盤計算（Python + pyswisseph；ephe/ 星曆檔）
