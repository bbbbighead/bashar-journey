...
照見文件（單一整合文件）：
  ...
  ＋「對應說明 · 牌與卦」——攤開彙整的工作底稿：點名關鍵牌與位置、
    本卦變卦與體用格局，說明結果與牌面、卦象的關聯（唯一出現牌名卦名的區塊）
```

重要規則：

- **牌面可見、解讀整合、依據透明**：玩家看得到抽出的九張牌與位置；彙整正文不逐牌說明、不逐卦解釋、不出現術語（prompt 硬性約束 + `integrate.js` 的 `sanitize` 最後防線）；但文件末尾的「對應說明」區塊會點名關鍵牌與卦，讓使用者看得到結果與依據的關聯。
