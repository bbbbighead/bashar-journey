...
網站最大的特色不是 AI 問四個問題，而是：**每一個問題，都來自 AI 根據「使用者描述＋雷諾曼＋梅花易數」建立的工作假說。**

```
首頁：輸入拖延情境
      ↓
占卜一：雷諾曼九宮格——36 牌抽 9 張，玩家逐一翻牌（牌面可見），
        位置語義（過去/現在/走向 × 想法/現實/潛意識）進入內部讀數
      ↓
占卜二：梅花易數——玩家憑直覺報三個 1–100 的數字（報數起卦；跳過則以時間起卦）
      ↓
AI 建立 3–5 個拖延機制假說（內部，不顯示）
      ↓
四個探索問題——一次一題、不分析、不解釋、只提問；
每一題都用來驗證／排除假說
      ↓
第五題（確認）——AI 整合成主要假說，說給使用者聽，
使用者裁決：是 / 部分是 / 不是（可補充）
      ↓
最後分析（五段式）：
...
- **假說不是診斷**：全程假說語氣，使用者的裁決（是/部分是/不是）真實影響最後分析——被否定的假說會被承認並重新框定。
- **牌面可見、正文無術語、依據透明**：九張牌與位置玩家看得到；分析正文不出現牌名卦名術語（prompt 約束 + `inquiry.js` 的 `sanitize`）；末尾「對應說明」區塊點名關鍵牌與卦，攤開工作底稿。
- **不預測、不決定論**：過程性語言；不提供醫療/法律/財務建議；不下診斷。
- **危機攔截**：入口與探索中途偵測自我傷害訊號即中止，顯示關懷資源頁。
- **離線後備（從簡）**：未設金鑰或 AI 失敗時，四題採規格範例題、確認與分析用規則式模板——體驗可完整走完，但假說驅動的個人化是線上（AI）路徑專屬。
...
每場探索約 7 次呼叫。假說品質是整站核心，故 probe 也用強模型：

- **hypothesize / probe ×4 / confirm**：Sonnet 5（或 OpenAI gpt-5.1）
- **analyze**（最後分析）：Opus 4.8（或 OpenAI gpt-5.1）

## 檔案結構

```
index.html               入口（首頁）＋全部畫面
css/calm.css             沉靜主題（深夜墨色、暖紙感、翻牌）
js/app.js                頂層流程控制（占卜 → 假說 → 探索 → 確認 → 分析）
js/engine/session.js     探索狀態 + localStorage 續玩
js/engine/inquiry.js     探索引擎（AI/離線雙路徑、對應說明、sanitize）
js/engine/lenormand.js   雷諾曼引擎（抽牌、九宮格語義、主題群集）
js/engine/meihua.js      梅花易數引擎（報數/時間起卦、本互變卦、體用生剋）
js/content/crisis.js     危機關鍵字攔截
js/content/templates.js  離線後備（規格範例題、假說陳述、分析素材）
js/ai/client.js          對 /api/insight 的 fetch 包裝
data/lenormand.js        36 牌內部詮釋 + 主題群集 + 宮位語義
data/hexagrams.js        64 卦內部詮釋 + 八卦五行 + 階段語義
api/insight.js           Vercel serverless 代理（hypothesize/probe/confirm/analyze）
