...
只設 `ANTHROPIC_API_KEY` 的話卡面文字產得出來、圖產不出來。

**對外開放與否在後台切**（`/admin.html` → 靈感牌卡分頁最上方那顆開關）。值存在 Redis
（`pi:oracleon`），端點每次請求都讀，所以按下去即時生效——不必改程式、不必重新部署。
**沒設定過＝關閉**：誤關只是少產一張卡，誤開會花錢。其餘可調的環境變數：

| 變數 | 預設 | 說明 |
|---|---|---|
| `ORACLE_ENABLED` | *（不設）* | **硬性的 kill switch，有設就贏。** 設了之後後台那顆開關會被鎖住（畫面會說明原因）。平常不要設——留空才能用後台即時開關。要交回後台控制就把這個變數整個刪掉再 Redeploy |
