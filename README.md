...
**對外開放與否只在後台切**（`/admin.html` → 靈感牌卡分頁最上方那顆開關），沒有對應的
環境變數。值存在 Redis（`pi:oracleon`），端點每次請求都讀，所以按下去即時生效——
不必改程式、不必重新部署。**拿不到就一律當關閉**：沒設定過、Redis 沒設定、Redis 掛了
都算關。方向是刻意的——誤關只是少產一張卡，誤開會花錢。其餘可調的環境變數：
