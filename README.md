...
「今天想獲得什麼靈感訊息？」——帶著一個想被啟發的主題（事業、創作、感情、人生方向……），親手從 36 張雷諾曼牌中選出 9 張、憑直覺報三個數起卦；系統把兩套系統視為不同觀測角度交叉比對，綜合成一則直接寫給你的靈感訊息。（西洋占星本命盤保留給進階版服務，第一階段不要求出生資料。）
...
占卜一：雷諾曼——36 張牌全數列出（牌背朝上、順序洗亂），
        使用者親手憑直覺選取 9 張（點選＝發光選取、再點＝取消）；
        不揭示牌面，選取順序對應內部九宮格 → 觀測「現實如何表現」
      ↓
...
## 西洋占星計算（api/astro.py・保留給進階版）

> 第一階段前台流程不包含占星、不蒐集出生資料；以下能力保留給「詳細進階報告」服務啟用時使用。
...
- **使用者親手選牌**：36 張牌背朝上全數列出，點選＝選取（發光框）、再點＝取消；選取順序決定內部九宮格位置，牌面不對使用者揭示。
- **兩源必須綜合**：訊息建立在雷諾曼與梅花易數的交集上，再對照主題落地；不逐牌逐卦解釋。
- **訊息不揭示出處**：正文不提雷諾曼／梅花易數／星盤，也不出現「幾套系統」「交叉比對」等方法論字眼——就是一封直接寫給諮詢者的信（prompt 約束 + `inquiry.js` 的 `sanitize` 最後防線）。結果頁文末索引列出九宮格 1–9 的牌卡與三個數字；另有「查看詳細進階報告」按鈕（結合牌卡、卦象與星盤的詳細報告解說——功能陸續開放）。
